...
public class TGDeviceEEGPowerLowBeta extends GenericSignal implements Runnable, SignalSettable {
    private static final String TAG = makeLogTag(TGDeviceEEGPowerLowBeta.class);

    @Override
    public void run() {
        TGDeviceHandler.getActivityViewContract().setLowBeta(level);
        LOGD(TAG, "Control signal - Low Beta wave: " + level);
    }
}
